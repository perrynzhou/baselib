                    ...
                    cstl_object_func *funcs)
{
  ...
  s->cur_size = 0;
  s->data =
      (cstl_object **)calloc(s->max_size, sizeof(cstl_object *));
  assert(s->data != NULL);
  if (funcs != NULL)
  {
    s->funcs =
...
cstl_stack *cstl_stack_alloc(uint64_t init_size,
                             cstl_object_func *funcs)
{

  cstl_stack *s =
      (cstl_stack *)calloc(1, sizeof(cstl_stack));
...
int cstl_stack_expand(cstl_stack *s)
{
  ...
  cstl_object **data = (cstl_object **)calloc(
      s->max_size * (1.5), sizeof(cstl_object *));
  assert(data != NULL);
  s->max_size *= 1.5;
