...
struct cstl_skiplist_node_t
{
  int64_t key;
  cstl_object *data;
  size_t level;
  struct cstl_skiplist_node_t *next_nodes[0];
};
...
static cstl_skiplist_node *
cstl_skiplist_node_alloc(int64_t key, cstl_object *obj)
{
  cstl_skiplist_node *node = NULL;
  if (obj != NULL)
  {
    int level = cstl_skiplist_random_level();
    node = calloc(1, sizeof(cstl_skiplist_node) +
                         sizeof(cstl_skiplist_node *) * level);
    node->level = level;
    node->data = obj;
    node->key = key;
...
int cstl_skiplist_init(cstl_skiplist *sl,
                       cstl_object_func *funcs)
{
  sl->head = calloc(1, sizeof(cstl_skiplist_node) +
                           sizeof(cstl_skiplist_node *) *
...
int cstl_skiplist_push(cstl_skiplist *sl, int64_t key,
                       cstl_object *obj)
{
